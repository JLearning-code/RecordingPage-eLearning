...
window.Script4 = function()
{
  ...
  function renderPlaybackFrame() {
    ...
    canvasCtx.lineWidth = 3;
    canvasCtx.strokeStyle = '#2ed573'; // Green wave for playback
    canvasCtx.beginPath();
...
// Helper to draw real-time waveform
function drawWaveform(analyser, strokeColor) {
  const bufferLength = analyser.frequencyBinCount;
  const dataArray = new Uint8Array(bufferLength);
  ...
  function renderFrame() {
    ...
    canvasCtx.fillStyle = '#1e1e2f'; // Background color of box
    canvasCtx.fillRect(0, 0, canvas.width, canvas.height);
...
// 2. Start Recording & Audio Context
navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
  ...
  window.customAudioRecorder.ondataavailable = e => window.audioChunks.push(e.data);
  window.customAudioRecorder.onstop = () => {
    const audioBlob = new Blob(window.audioChunks, { type: 'audio/mp3' });
    window.recordedAudioUrl = URL.createObjectURL(audioBlob);
  };
